...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'email_logs') THEN
        EXECUTE 'CREATE POLICY "System can manage email logs" ON public.email_logs FOR ALL USING (true) WITH CHECK (true)';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'email_templates') THEN
        EXECUTE 'CREATE POLICY "Users can view email templates" ON public.email_templates FOR SELECT USING (auth.role() = ''authenticated'')';
        EXECUTE 'CREATE POLICY "System can manage email templates" ON public.email_templates FOR ALL USING (true) WITH CHECK (true)';
    END IF;
END $$;
